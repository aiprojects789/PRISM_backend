...
async def generate_recommendations(
    ...
    try:
        ...
        profile_doc = db.collection("profiles").document(current_user.uid).get()
...
async def get_recommendation_history(current_user: Any = Depends(get_current_user)):
    ...
    try:
        db = get_firestore_client()
        recommendations = db.collection("recommendations").where("user_id", "==", current_user.uid).stream()
...
async def get_recommendation_details(recommendation_id: str, current_user: Any = Depends(get_current_user)):
    ...
    try:
        ...
        if rec_data["user_id"] != current_user.uid:
            raise HTTPException(
